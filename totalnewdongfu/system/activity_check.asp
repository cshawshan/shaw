...
<div id="contain">
	<div>
		...
		<span>这里能显示所有立项申请</span>
	</div>
	<br /><br /><br />
		<table>
			<tr><form method="post"><td colspan="9">
				学院/组织：<select name="academy">
<% session("academy_check")=request.Form("academy")
   if session("academy_check")<>"" then
%>
					<option><%=session("academy_check")%></option>
<% else
%>
<% end if %>
					<option value="">所有</option>
					<!--#include file="../system/academy.asp"--></select>


				审核情况：<select name="activity_check">
...
<% end if %>
			...
			<tr align="center">
				...
				<td>
					<form method="post" action="checkcode.asp">
						<input type="submit" value="通过(公费)" />
						<input type="hidden" name="val" value="1" />
						<input type="hidden" name="id" value="<%=rs("ID")%>"/>
					</form>
					<form method="post" action="checkcode.asp">
						<input type="submit" value="通过(自费)" />
