...
%>
   ...
   sql="select * from latestnews where news_sender='"&user&"' order by id desc"%>
...
<tr>
   ...
   <%do while not rs.eof %>
	      ...
	      <td><a href="news_lview.asp?category=latestnews&id=<%=rs("id")%>">详细</a></td>
              <td><a href="news_edit.asp?category=latestnews&id=<%=rs("id")%>">编辑</a></td>
              <td><a href="news_del.asp?category=latestnews&id=<%=rs("id")%>" onClick="return confirm('删除后无法恢复,确定要删除吗')">删除</a></td>
              </tr>
   <% rs.movenext %>
   <% loop %>
